...
$e = fn($s) => View::e($s);
?>
<a class="back" href="/admin/subscribers">← Abonnés</a>
<div class="two-col">
  <div class="panel">
    <h2><?= $e($sub['email']) ?></h2>
    <dl class="defs">
      ...
      <dt>Confirmé</dt><dd><?= $sub['confirmed'] ? 'Oui' : 'Non' ?></dd>
  ...
  <div class="panel">
    ...
    <form method="post" action="/admin/subscribers/<?= (int)$sub['id'] ?>">
      ...
      <label class="chk"><input type="checkbox" name="confirmed" value="1" <?= $sub['confirmed'] ? 'checked' : '' ?>> Confirmé</label>
    ...
    <form method="post" action="/admin/subscribers/<?= (int)$sub['id'] ?>/delete" onsubmit="return confirm('Supprimer cet abonn\u00e9 ?')">
      <?= Csrf::field() ?>
      <button class="btn btn-ghost full" type="submit">Supprimer l'abonné</button>
    </form>
  </div>
</div>
